  ...
  <div>
    ...
    <div class="max-w-3xl space-y-4">
      ...
      <div class="card space-y-3">
        ...
        <p v-if="!hasPassword" class="text-xs text-amber-700 bg-amber-50 rounded-sm px-2 py-1">
      ...
      <div v-if="restoreTarget" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
        <div class="bg-soft rounded-md shadow-xl w-full max-w-md p-6 space-y-4">
          ...
          <p class="text-sm text-ink/80">
            Backup: <code class="font-mono text-xs bg-bg-brand px-1 rounded-sm">{{ restoreTarget }}</code>
          </p>
        ...
        <div class="bg-soft rounded-md shadow-xl w-full max-w-md p-6 space-y-5">
          ...
          <div v-if="progressError" class="space-y-4">
            <div class="flex items-start gap-3 text-red-600 bg-red-50 rounded-lg px-3 py-3">
              <svg class="w-5 h-5 shrink-0 mt-0.5" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div v-else class="space-y-4">
            ...
            <div class="space-y-2">
              <div v-for="step in RESTORE_STEPS" :key="step.key"
                ...
                <!-- Erledigt -->
                <svg v-if="stepIsDone(step.key)" class="w-3.5 h-3.5 text-emerald-500 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                  <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clip-rule="evenodd" />
                </svg>
                <!-- Aktiv (pulsierend) -->
                <div v-else-if="stepIsActive(step.key)"
                     class="w-3.5 h-3.5 rounded-full border-2 border-primary-500 shrink-0 animate-pulse" />
                <!-- Ausstehend -->
                <div v-else class="w-3.5 h-3.5 rounded-full border border-sand shrink-0" />
            ...
            <p v-if="progressStep === 'db_restore'"
               class="text-xs text-amber-700 bg-amber-50 rounded-sm px-2.5 py-1.5">
